...
FEATURES = [
    ...
    ("capability", "Numeric corner input", "no", "yes: positions on Illustrator's ruler, or offsets from the undistorted corner, with a live preview; one undo step", "yes",
     [("numeric", "OK with nothing typed changes nothing"), ("numeric", "...and adds no undo step"), ("numeric", "an offset of 25 pt"), ("numeric", "...as exactly one undo step"),
      ("numeric", "...which one Undo takes back"), ("numeric", "a position of 200 pt"), ("numeric", "Cancel after typing"), ("numeric", "...and leaves no undo step"),
      ("numeric", "the up arrow in a field"), ("numeric", "Enter in a field"), ("corner-dialog", "OK with nothing typed"), ("corner-dialog", "Undistort puts every corner"),
      ("manual", "double-click on the tool icon opens")]),
    ("capability", "Numbers typed as Illustrator's own fields take them: either decimal separator, units, expressions, full precision", "whole points", "yes", "nothing stored",
     [("numeric", "a field typed '12.5'"), ("numeric", "a field typed '12,5'"), ("numeric", "a field typed '1 in'"), ("numeric", "a field typed '3 mm'"),
      ("numeric", "a field typed '10+5'"), ("numeric", "a field typed '-15.5 pt'"), ("numeric", "a field typed '12.3456789'"), ("numeric", "a field typed 'abc'"),
      ("numeric", "the dialog's positions are the ruler's"), ("numeric", "round-trips exactly")]),
    ("capability", "Arrow keys move the selected corner by Illustrator's keyboard increment, Shift by ten", "no", "yes: click a handle to select it; one undo step per key", "yes",
     [("numeric", "with a corner selected, Right then Up"), ("numeric", "...one undo step per key"), ("manual", "Right three times and Shift+Right once"),
      ("manual", "...one undo step per key press"), ("manual", "does not also nudge the art"), ("manual", "keyboard increment is the one"), ("manual", "a real click on a handle selects")]),
    ("capability", "The tool's icon and the About window follow Illustrator's theme", "n/a", "yes", "nothing stored",
     [("manual", "the tool icon matches"), ("manual", "opens the templated About window")]),
    ...
    ("capability", "Axis lock, as Free Transform's Shift in its Free Distort mode", "no", "Shift", "yes: eight numbers",
     [("editor", "axis (Shift)"), ("mathtest", "Free Transform's corners: Ctrl+Shift")]),
    ("capability", "Converging sides, the corners of Free Transform's Perspective Distort", "no", "Shift+Alt", "yes: eight numbers",
     [("editor", "converging (Shift+Alt): a mostly horizontal"), ("editor", "converging (Shift+Alt): a mostly vertical"),
      ("mathtest", "Free Transform's corners: Ctrl+Alt+Shift"), ("mathtest", "Perspective Distort mode")]),
    ("capability", "Symmetric, as Free Transform's Alt in its Free Distort mode", "no", "Alt", "yes: eight numbers",
     [("editor", "symmetric (Alt)"), ("mathtest", "Free Transform's corners: Ctrl+Alt,")]),
    ("capability", "Free Transform's keys, measured by hand, reproduced at the corners", "n/a", "yes, with Ctrl ignored, so Free Transform habits carry over", "nothing stored",
     [("mathtest", "Free Transform's corners")]),
    ("unrepresentable", "True perspective foreshortening, which Free Transform's distort modes apply to the interior", "no", "no: the renderer is bilinear", "cannot be", []),
    ("capability", "Snapping, through Illustrator's own Smart Guides", "no", "yes: art, guides, grid, artboards, and the undistorted corners, center, and edges; tolerance in screen pixels; follows View > Smart Guides", "yes",
     [("snap", "lands on the anchor exactly"), ("snap", "lands there exactly"), ("snap", "undistorted center"), ("snap", "far from every target"),
      ("snap", "never breaks the axis lock"), ("snap", "too far to snap"), ("snap", "still snaps"), ("snap", "nothing snaps")]),
    ("capability", "Never writes into something it is no longer editing: object deleted, effect removed or moved, document closed, undo and redo", "n/a", "yes", "nothing stored",
     [("lifecycle", "object is deleted"), ("lifecycle", "Free Distort is removed"), ("lifecycle", "moved ahead of it"), ("lifecycle", "after an Undo"),
      ("lifecycle", "Undo, Undo, Redo"), ("lifecycle", "closing the editor's document"), ("lifecycle", "in the document opened next")]),
    ("capability", "Repeated use without crashing or leaking window resources", "n/a", "yes", "nothing stored",
     [("lifecycle", "cycles of open"), ("lifecycle", "without GDI or USER objects growing")]),
    ...
    ("limit", "Illustrator 30.7.0 can crash creating a document after documents were closed under scripting", "the same", "the same: it reproduces with this plugin removed, at the offset of a known Illustrator document churn crash (crash-sequence evidence)", "n/a", []),
