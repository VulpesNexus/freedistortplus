...
.DESCRIPTION
    Needs Illustrator running with EnhancedFreeDistort.aip loaded. Exported
...
Say ('Enhanced Free Distort -- persistence and cost, {0}' -f (Get-Date -Format 'yyyy-MM-dd HH:mm'))
...
$edited = Quads
$drawn = Invoke-Efd 'app.redraw(); EFD.bounds("pent");'
Say "edited: $edited; drawn $drawn"

# ---- copy and paste --------------------------------------------------------------------------

Invoke-Efd 'app.copy(); app.paste(); var d = EFD.doc(); var p = d.selection[0]; p.name = "pasted"; EFD.selectOnly("pasted"); app.redraw(); "pasted";' | Out-Null
$pasted = Quads
Check 'clipboard' 'copy and paste carry the edited Free Distort, all sixteen numbers' $edited $pasted ($pasted -eq $edited)
$pastedDrawn = Invoke-Efd 'EFD.bounds("pasted");'
...
Check 'export' 'the PDF names no part of this plugin' 'none' $(if ($pdfBytes -match 'EnhancedFreeDistort|VulpesNexus') { 'found' } else { 'none' }) (-not ($pdfBytes -match 'EnhancedFreeDistort|VulpesNexus'))
# Put the probe document back in place, as the fixtures expect.
Invoke-AiScript "(function(){ for (var i = app.documents.length - 1; i >= 0; i--) { if (app.documents[i].name === 'efd-persistence.pdf') { app.documents[i].close(SaveOptions.DONOTSAVECHANGES); } } return 'closed'; })();" | Out-Null
Initialize-AiSession | Out-Null
Invoke-Efd 'EFD.clear(); EFD.pentagon("pent"); EFD.selectOnly("pent");' | Out-Null
Send-AiMessage 'fd append' | Out-Null
Send-AiMessage 'editor refresh' 'measure' | Out-Null
...
$svg = Join-Path $scratch 'efd-persistence.svg'
Invoke-Efd ("(function(){{ var d = EFD.doc(); var o = new ExportOptionsSVG(); d.exportFile(new File('{0}'), ExportType.SVG, o); return 'svg'; }})();" -f ($svg -replace '\\', '/')) | Out-Null
...
Check 'export' 'the SVG names no part of this plugin' 'none' $(if ($svgText -match 'EnhancedFreeDistort|VulpesNexus') { 'found' } else { 'none' }) (-not ($svgText -match 'EnhancedFreeDistort|VulpesNexus'))
...
Invoke-Efd 'EFD.clear(); EFD.pentagon("first"); EFD.pentagon("second", 300, 0); EFD.selectOnly("first");' | Out-Null
Send-AiMessage 'tool select' 'Adobe Select Tool' | Out-Null
Invoke-AiScript 'app.executeMenuCommand("VulpesNexus Enhanced Free Distort"); "invoked";' | Out-Null
...
Invoke-Efd 'EFD.selectOnly("second");' | Out-Null
...
function Time-Drag([string] $fixture, [string] $name, [int] $steps) {
    Invoke-Efd ("EFD.clear(); EFD.{0}('{1}');" -f $fixture, $name) | Out-Null
    ...
    Invoke-Efd ("EFD.selectOnly('{0}'); app.redraw();" -f $name) | Out-Null
    Send-AiMessage 'fd append' | Out-Null
