...
function Result([string] $name) { ((Invoke-Fdp ("FDP.sourceAndResult('{0}');" -f $name)) -split "`r?`n" | Where-Object { $_ -like 'R*' }) -join '|' }
...
Say ('FreeDistort+ -- dictionary semantics, {0}' -f (Get-Date -Format 'yyyy-MM-dd HH:mm'))
...
$sourceRows = ((Invoke-Fdp 'FDP.sourceAndResult("pent");') -split "`r?`n" | Where-Object { $_ -like 'S*' }) -join '|'
...
Send-AiMessage 'set param' '0|fdpProbeKey|real|123' | Out-Null
...
Record 'keys' "a key Adobe does not know, after Adobe's own edit path commits" $(if ($afterVanilla -match 'fdpProbeKey') { 'kept' } else { 'dropped' })
Send-AiMessage 'delete param' '0|fdpProbeKey' | Out-Null
...
Fresh
$stored = Read0
Invoke-Fdp 'FDP.named("pent").translate(50, 20); app.redraw(); "moved";' | Out-Null
Check 'art changes' 'moving the art does not touch the dictionary' $stored (Read0) ((Read0) -eq $stored)
$bounds = Invoke-Fdp 'FDP.bounds("pent");'
...
Fresh
Invoke-Fdp 'FDP.named("pent").resize(150, 150, true, true, true, true, 150, Transformation.CENTER); app.redraw(); "scaled";' | Out-Null
...
Fresh
Invoke-Fdp 'FDP.named("pent").rotate(30); app.redraw(); "rotated";' | Out-Null
$rotated = Invoke-Fdp 'FDP.bounds("pent");'
Silent
...
Fresh
Invoke-Fdp 'var p = FDP.named("pent"); var a = p.pathPoints[2]; a.anchor = [440, 260]; a.leftDirection = [440, 260]; a.rightDirection = [440, 260]; app.redraw(); "reshaped";' | Out-Null
...
Invoke-Fdp 'FDP.clear(); FDP.stroked("s"); FDP.selectOnly("s");' | Out-Null
Send-AiMessage 'fd append' | Out-Null
...
Invoke-Fdp 'FDP.clear(); FDP.pentagon("t"); FDP.selectOnly("t");' | Out-Null
